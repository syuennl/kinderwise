...
function getAvailableOptions($conn, $entity) {
    ...
    switch ($entity) {
        ...
        case 'subject':
            ...
            break;
        case 'student':
            // Get available classes (that haven't reached max capacity)
            $options['classes'] = [];
            $query = "SELECT c.classCode 
                     FROM class c 
                     WHERE (SELECT COUNT(*) FROM student s WHERE s.classCode = c.classCode) < 15";
            $result = mysqli_query($conn, $query);
            while ($row = mysqli_fetch_assoc($result)) {
        ...
        <form method="POST" id="addSettingForm" onsubmit="return validationForm()">
            ...
            <?php break;
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="assessmentTitle" required maxlength="50" oninput="validationForm()">
                    <div class="hint">Maximum 50 characters</div>
                </div>
