...
$input_types = [
    ...
    'assessmentType' => ['type' => 'text', 'maxlength' => 50],
    'deadline' => ['type' => 'date'],
    'status' => ['type' => 'select', 'options' => ['No submmission', 'Submitted']],
        ...
        <form method="POST" id="editSettingForm" onsubmit="return validateForm()">
            <?php foreach ($columns[$entity] as $index => $column): ?>
                <div class="form-group">
                    ...
                    <?php
                    // Primary key field (first column) is always readonly
                    if ($index === 0): ?>
                        <input type="text" value="<?php echo htmlspecialchars($current_data[$column]); ?>" >
                    <?php else:
                        // Determine the input type and attributes
                        $input_type = $input_types[$column] ?? ['type' => 'text'];
                        
                        switch ($input_type['type']):
                            case 'select':
                                if (isset($available_options[strtolower($column . 's')])): ?>
                                    <select name="<?php echo $column; ?>" required>
                                        <?php foreach ($available_options[strtolower($column . 's')] as $option): 
                                            $value = is_array($option) ? $option['id'] : $option;
                                            $display = is_array($option) ? $option['name'] : $option;
                                        ?>
